...
describe('WhatsApp phone numbers API', () => {
  ...
  it('Request a verification code sent via SMS in english', async () => {
    scope = (0, nock_1.default)(`https://${sdkConfig.WA_BASE_URL}`).post(`${basePath}/request_code`).delay(200).delayBody(200).delayConnection(200).reply(200, defaultPhoneNumbersResponseObjectBody);
    const body = {
  ...
  it('Send verification code of "00000" to verify phone number', async () => {
    scope = (0, nock_1.default)(`https://${sdkConfig.WA_BASE_URL}`).post(`${basePath}/verify_code`).delay(200).delayBody(200).delayConnection(200).reply(200, defaultPhoneNumbersResponseObjectBody);
    const body = {
      code: '00000'
    };
