...
describe('WhatsApp Messages API', () => {
  ...
  it('messages class instance', () => {
    expect(wa.messages).toBeInstanceOf(messages_1.default);
  });
  it('Send text message', async () => {
    scope = (0, nock_1.default)(`https://${sdkConfig.WA_BASE_URL}`).post(`${basePath}/messages`).delay(200).delayBody(200).delayConnection(200).reply(200, defaultMessagesResponseBody);
    const response = await wa.messages.text({
      body: 'test'
    }, testRecipient);
    expect(await response.responseBodyToJSON()).toStrictEqual(defaultMessagesResponseBody);
    scope.isDone();
  });
