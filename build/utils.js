...
export const importConfig = (senderNumberId, configToImport) => {
    ...
    const config = {
        ...
        [WAConfigEnum.AppId]: configToImport[WAConfigEnum.AppId] || process.env.M4D_APP_ID || '',
        [WAConfigEnum.AppSecret]: process.env.M4D_APP_SECRET || '',
        [WAConfigEnum.PhoneNumberId]: senderNumberId || parseInt(process.env.WA_PHONE_NUMBER_ID || ''),
        [WAConfigEnum.BusinessAcctId]: configToImport[WAConfigEnum.BusinessAcctId] ||
            process.env.WA_BUSINESS_ACCOUNT_ID ||
            '',
        [WAConfigEnum.APIVersion]: configToImport[WAConfigEnum.APIVersion] ||
            process.env.CLOUD_API_VERSION ||
            '',
        [WAConfigEnum.AccessToken]: configToImport[WAConfigEnum.AccessToken] ||
            process.env.CLOUD_API_ACCESS_TOKEN ||
            '',
        [WAConfigEnum.WebhookEndpoint]: process.env.WEBHOOK_ENDPOINT || '',
        [WAConfigEnum.WebhookVerificationToken]: process.env.WEBHOOK_VERIFICATION_TOKEN || '',
        [WAConfigEnum.ListenerPort]: parseInt(process.env.LISTENER_PORT || '') || DEFAULT_LISTENER_PORT,
        [WAConfigEnum.MaxRetriesAfterWait]: parseInt(process.env.MAX_RETRIES_AFTER_WAIT || '') ||
            DEFAULT_MAX_RETRIES_AFTER_WAIT,
        [WAConfigEnum.RequestTimeout]: parseInt(process.env.REQUEST_TIMEOUT || '') || DEFAULT_REQUEST_TIMEOUT,
        [WAConfigEnum.Debug]: process.env.DEBUG === 'true',
    };
    LOGGER.log(`Configuration loaded for App Id ${config[WAConfigEnum.AppId]}`);
    return config;
};
export const generateXHub256Sig = (body, appSecret) => {
    return crypto
